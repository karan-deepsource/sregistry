...
def all_collections(request):
    ...
    collections = (
        ...
        .order_by("-star__count")[:limit]
    )

    # private that the user can view
    private_collections = [
        x
        for x in Collection.objects.filter(private=True)
    ...
    if request.method == "POST":
        ...
        if private is not None and PRIVATE_ONLY is False:
            ...
            if collection.private != private:
                collection = _change_collection_privacy(
                    request=request, collection=collection, make_private=private
                )

    context = {"collection": collection, "edit_permission": edit_permission}
...
def change_collection_privacy(request, cid, make_private=True):
    ...
    collection = get_collection(cid)
    collection = _change_collection_privacy(
        request=request, collection=collection, make_private=make_private
    )
        ...
        if request.method == "POST":
            ...
            if add_contribs not in unset:
                collection = _edit_contributors(
                    userids=add_contribs, collection=collection
                )
            if add_owners not in unset:
                collection = _edit_contributors(
                    userids=add_owners, collection=collection, level="owner"
                )

            if remove_contribs not in unset:
                collection = _edit_contributors(
                    userids=remove_contribs, collection=collection, add_user=False
                )
